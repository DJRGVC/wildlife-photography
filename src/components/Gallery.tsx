...
function renderCaption(p: GalleryPhoto, showTitle: boolean): string {
  const meta: string[] = [];
  ...
  if (showTitle && p.animal) {
    parts.push(`<div class="caption__title">${escapeHtml(p.animal)}</div>`);
  }
          ...
          image: (imgProps, ctx: RenderImageContext<Photo>) => {
            ...
            return (
              <a
                ...
                aria-label={p.animal || p.fileName}
                target="_blank"
                rel="noreferrer"
    ...
    return (
      <section className="gallery gallery--empty" aria-label="Photography gallery">
        <p className="gallery__empty">
          ...
          <code>src/photography/misc/</code> and the gallery will appear here.
        </p>
...
const galleryStyles = `
  ...
  .gallery__inner + .gallery__sep,
  .gallery__sep + .gallery__inner {
    margin-top: 0;
  }
  .gallery__sep {
    display: flex;
